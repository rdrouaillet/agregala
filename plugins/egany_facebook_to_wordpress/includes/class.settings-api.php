...
/**
 * EGANY Settings API wrapper class
 *
 * @version 1.0
 *
 ...
 */
if ( !class_exists( 'Egany_Settings_API' ) ):
class Egany_Settings_API {
    ...
    function admin_init() {
		...
		$count_ids = 1; 
		$FBWPadmin = new Egany_FB_Group_To_WP_Admin(); 
		for($i = 2; $i < 500; $i++){  
			$default = ''; 
			$page_group_id = esc_attr( $this->get_option( 'page_group_id'.$i, 'fb2wp_page_group', $default ) ); 
			...
			if(trim($page_group_id) != ''){ 
				...
				$arrFields = $FBWPadmin->get_sf_facebook_info_account($i);  
				$arrFields = array_merge( 
					$FBWPadmin->get_sf_fb_id_and_type($i) 
					,$arrFields
				); 
				foreach($arrFields as $field){  
					$this->add_field('fb2wp_page_group',   
						$field   

					); 
			
				}
			} // end if: trim page_group_id
		}
                ...
                <?php foreach ( $this->settings_sections as $form ) { ?>
                    <div id="<?php echo $form['id']; ?>" class="group">
                        <form method="post" action="options.php">
							...
							<div id='<?php echo $form['id']; ?>_fields'>  
                            ...
                            <div class="btn_panel">
								...
								<?php if( $form['id'] == 'fb2wp_page_group'){ ?>
								<a id="add_page_group" href="javascript:;" target="_blank" class="button" title="Add 1 more page/group info. &amp; connection info.">Add Facebook Info.</a>  
								<?php } ?>
								<!-- <a href="<?php echo home_url().'/?fb2wp_type=page' ?>" target="_blank" class="button hidden" title="Must-have do Save Changes successfully before running" >Import Page</a> -->
